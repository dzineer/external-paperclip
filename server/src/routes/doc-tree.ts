  ...
  router.post("/companies/:companyId/agents/:agentId/train-brain", async (req, res) => {
    ...
    try {
      ...
      for (const file of driveFiles) {
        try {
          let content = "";
          const tmpPath = join(tmpdir(), `brain-${Date.now()}-${file.id}`);

          const tmpFilename = `brain-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
          const tmpDir = tmpdir();
            ...
            try {
              await execFileAsync(
                ...
                ["@googleworkspace/cli", "drive", "files", "export",
                  ...
                  "--output", tmpFilename],
                { timeout: 60_000, env: { ...process.env }, cwd: tmpDir },
              );
              content = await readFile(join(tmpDir, tmpFilename), "utf-8");
            } catch {
              continue;
            } finally {
              await unlink(join(tmpDir, tmpFilename)).catch(() => {});
            }
          } else {
            // Regular files: download (text, markdown, json, csv, docx, pdf)
            const skipMimes = ["image/", "video/", "audio/"];
            const shouldSkip = skipMimes.some((t) => file.mimeType.startsWith(t));
          ...
          // Chunk large documents (Graphiti/OpenAI fails on >8K token outputs)
      ...
      res.status(502).json({ error: `Failed to read Drive folder: ${(err as Error).message}` });
      return;
    }
    ...
    await logActivity(db, {
      ...
      entityType: "agent",
      entityId: agentId,
      details: { driveFolderId, driveFolderName, filesIngested: ingested, totalFiles },
    });

    res.json({ trained: true, documentsIngested: ingested, totalFiles });
  });
